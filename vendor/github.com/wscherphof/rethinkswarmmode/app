...
if [ "$?" = "0" ]; then
    for env in "${ENVS[@]}"; do
        ...
        ${DOCKER} service update --detach=true --env-add ${env} ${NAME}
    done
    echo "* updating image to ${TAG}..."
	${DOCKER} service update --detach=true --image ${TAG} ${NAME}
    echo "* scaling..."
	...
	${DOCKER} service create --detach=true --mount src=appdata,dst=/appdata --name ${NAME} --replicas ${REPLICAS} ${ENVIRONMENT} --network ${NETWORK} ${PUBLISH} ${TAG}
fi
